...
def main():
    ...
    if config.qa_type == 'classification':
        model =  TCP_QA_Classification(args, config)
    else:
        model =  TCP_QA_Multichoice(args, config)
    ...
    if config.qa_type == 'classification':
        trainer = TCP_QA_Classification(args, config, model_engine, optimizer, scheduler, dataloader_train, dataloader_vals['QADataset-val'])
    else:
        trainer = TCP_QA_Multichoice(args, config, model_engine, optimizer, scheduler, dataloader_train, dataloader_vals['QADataset-val'])
